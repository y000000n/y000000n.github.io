...
def extract_terms_ai(text):
    ...
    instructions = """입력문을 처음부터 끝까지 한 번만 검토해 한일·일한 통역 준비에 필요한 고유명사와 전문용어를 추출한다.

포함 규칙:
- 제외 대상이 아닌 모든 인명을 빠짐없이 추출한다. 인명과 소속·직책이 함께 나오면 원문의 연속된 전체 표현을 하나의 '인명·소속·직책' term으로 만들고 인명·직책을 따로 중복하지 않는다. 원문에 없는 정보는 추정하지 않는다.
- 정부부처가 아닌 기업·위원회·협회·노조·재단·연구소·대학·정당·단체·국제기구·팀, 부서명과 팀명은 반드시 포함한다. 결합된 인명 term에 든 조직명도 별도 조직 항목으로 유지한다.
- 정식 법률·협정·정책·사업·기술명과 '생산가능인구'처럼 별도 정의나 정형 번역을 준비해야 하는 전문개념을 포함한다.

제외 규칙:
- 국가명과 서울·도쿄·뉴욕·워싱턴 같은 유명 지명, 정부부처·대통령실 및 이름 없는 부처+장관은 제외한다. 단, 사람 이름이 있는 장관은 부처+이름+직책으로 포함한다.
- 한국·일본의 대통령·총리, 5·18 민주화운동, GDP·GNP·닛케이 지수 등 기초 시사용어를 제외한다.
- 일반명사와 일반 표현은 반복되거나 중요해도 제외한다. 정부·명예회복·특별법·피해자·유가족·진상규명·지원·보상·법률·정책·사업 같은 일반 부분을 정식 명칭에서 떼어내지 않는다.
- 국가·지명이 포함된 정식 조직명·법률명·협정명·사업명은 보존하되 국가·지명만 따로 출력하지 않는다.

출력 규칙:
- term은 원문에 실제로 있는 연속 문자열을 그대로 쓰고 중복하지 않는다. 항목이 없으면 빈 배열을 반환한다.
- 한국어 term은 일본어로, 일본어 term은 한국어로 번역한다. 영문은 주변 문장의 반대 언어로 번역하며 불확실한 정식명은 널리 쓰이는 표기나 음역을 쓴다.
- context는 기사 속 의미·역할을 한국어로 짧게 쓰고 position은 최초 등장 문자 위치에 가까운 정수로 쓴다."""
    result = call_openai_structured(instructions, text, schema, "terminology_extraction", "medium")
    result["terms"] = refine_extracted_terms(result.get("terms", []), text)
    return result
...
def terminology_extraction():
    ...
    if st.button("용어 추출", type="primary", use_container_width=True):
        ...
        if not article_url.strip() and not text.strip(): st.error("신문기사 URL 또는 분석할 텍스트를 입력해주세요.")
            ...
            try:
                with st.spinner("인명·직책·조직명과 전문용어를 추출하고 있습니다…"):
